...
package object serialization {
  ...
  implicit def toNotificationType(channel: String): Option[NotificationType] =
    NotificationType.fromName(channel)

  implicit def toChannels(channels: List[String]): List[NotificationType] =
    channels.flatMap(toNotificationType)
}
